...
`python -m unittest identity.tests.test_services identity.tests.test_service_readiness` exercises the operator readiness command and actual user-systemd dependency transactions. It requires a running user systemd manager (`XDG_RUNTIME_DIR`). Isolated units retain the shipped dependency/restart/maintenance graph; controlled process fixtures replace privileged cloud work, use real kernel locks and emit real readiness notifications. Hosted acceptance separately verifies the installed privileged processes and actual runtime tunnel.
