...
The [database starter](https://a-d1b0371db71144778ec01df5.small-cloud.monkeysees.one) and [isolation peer](https://a-1040f9a86e414921a8d58d52.small-cloud.monkeysees.one) remain creator-only and occupy two active slots. The earlier `publishing-probe` app is unchanged. This increment required no platform service/configuration deployment: it uses the existing database provisioning and runtime handoff. Idle lifecycle, reset/deletion and broader deployment-failure/schema-repair acceptance remain their separate tickets.
