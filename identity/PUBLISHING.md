...
Apps serve HTTP on `0.0.0.0:$PORT` (`8080` in this deployment) and implement `GET /_small-cloud/ready`. The platform runs them as UID 65532 with a read-only root, gVisor, 0.5 CPU, 512 MiB memory and 128 tasks. `/tmp` is temporary and consumes the same memory budget. Readiness has 120 seconds; it must report 200 only after initialization. Database-backed app behavior and its persistent-data acceptance belong to #6. No backup or recovery guarantee exists: use only disposable data.
