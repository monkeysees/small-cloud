...
`--timeout SECONDS` is a positive integer (default 900) bounding observation after acceptance, excluding source validation and upload. Observation timeout exits 6; Ctrl-C during submission or observation exits 130. Neither cancels accepted work. Both return the request ID, operation ID, pinned workspace ID, last observed state, `work_continues: true` and `next_command` in JSON error details; readable errors explain the continuing work and show the command. Execute that inspection command before retrying. When submission loses its response, the outcome is unknown and recovery uses the original request ID instead of an unconfirmed operation ID. Reusing that request ID in the same workspace with identical source/metadata recovers the original operation; different input conflicts. A default-wait retry observes its current state, while `--no-wait` still reports only its original acceptance.

Failed deployments include the failed phase, a bounded redacted log excerpt and exact inspection commands in human/JSON output. Diagnostic enrichment has a separate three-second bound; Ctrl-C during enrichment stops the read and retains the known deployment failure's exit. Unavailable logs never establish success. Persistent pending work, service-side reconciliation errors or interrupted runtime collection require operator escalation using the original identities, not another deployment. See [failure diagnosis and tail snapshots](DIAGNOSTICS.md) and `small-cloud guide publishing`.
