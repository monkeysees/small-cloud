...
COMMANDS = [
    ...
    command('app deploy', 'Upload a Dockerfile source folder and wait until ready; --no-wait returns acceptance only.',
            'app deploy . --name example --description demo --json', 'Creator; updates require app ownership.',
            'operation_id, request_id, workspace_id, app, url, active_deployment_id and next_command; state: succeeded with ready: true by default, accepted with ready: false for --no-wait. Failure details include failed_phase, bounded redacted diagnostics, next_steps, guidance and operator_escalation. Timeout/interruption retains IDs, last state, work_continues, reconciliation_required and next_command; unknown submission uses outcome_unknown and request inspection. --dry-run returns included/excluded paths and total_bytes.',
            'Uploads source and builds; consumes allowance. Waits up to --timeout after acceptance; timeout or Ctrl-C stops observation only. Dry run is local only.',
            [argument('folder', 'Local source folder containing Dockerfile.'),
             ...
             argument('--dry-run', 'Validate and list upload contents without uploading.', action='store_true'),
             argument('--no-wait', 'Return accepted (ready: false) immediately; remote work continues.', action='store_true'), TIMEOUT],
    ...
    command('app logs', 'Read a bounded build or runtime log snapshot (no live following).',
            ...
            inputs=[APP, argument('--source', 'Log stream.', choices=['build', 'runtime'], required=True),
                    ...
                    argument('--cursor', 'Continuation from an earlier snapshot; exclusive with --since and --tail.', exclusive='position'),
                    argument('--tail', 'Read the most recent retained entries in chronological order; no cursor continuation.', action='store_true'),
    ...
    command('operation status', 'Reconcile existing work by ID or original request UUID; diagnose failed deployments without resubmitting.',
            'operation status op_FROM_DEPLOY --json', 'Operation owner or workspace administrator.',
            'id, operation_id, kind, state, app, deployment_id, request_id, created_at, updated_at, completed_at, error, cleanup_pending. Unfinished/failed deployments include recovery with pinned inspection, guidance and operator_escalation; failures add failed_phase, diagnostics and next_steps. Exit 0 means inspection succeeded, not deployment success.',
