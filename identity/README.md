...
Create a Google OAuth **web application** client with authorized redirect URI `https://small-cloud.monkeysees.one/auth/callback`. Configure consent and any required test-user access in the Google project. No Google client secret belongs in Git, CLI arguments, browser JavaScript or an app. The service requests only `openid email profile` and validates signature, issuer, audience, expiry, nonce, verified email and authorized party, with state and PKCE protecting the authorization-code exchange.
