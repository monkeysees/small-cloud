...
Local checks do not establish deployed acceptance. The Google web client, administrator/creator sign-in, retained credential and revocation checks above are now verified against the live HTTPS endpoint, with app routes still returning 503. Before closing #4, complete the deployed alternate-identity checks for unadmitted users and ordinary members/creators without administrator authority; preserve the distinction from the passing local matrix. Publishing, directory/app authorization, creator removal and integrated infrastructure verification remain #5, #7, #14 and #15.
