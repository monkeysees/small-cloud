...
Management requests use `Authorization: Bearer …` and mutation header `X-Request-ID`; do not place either credential or OAuth code in command arguments or access logs. Browser cookies are Secure, HttpOnly, SameSite=Lax, host-only `__Host-` cookies. Only the approval form accepts browser-authenticated mutations, requiring both CSRF token and matching Origin. CLI management routes never accept browser cookies as bearer credentials. Future app origins must remain separate; this service does not forward requests or credentials to apps.
...
Local verification on 2026-09-08 passed: 12 identity acceptance tests, 80 infrastructure tests, four hosting-fixture tests, the production Pyright check, wheel installation/CLI entry-point smoke check, and the PostgreSQL 16 own-database/cross-database isolation probe. Independent Standards review has no remaining findings; Spec review retains the live acceptance gap below.

Local checks do not establish Google project configuration or deployed acceptance. Before closing #4, verify the configured Google web client and sole administrator, install on the control host, and repeat admitted/unadmitted/member/creator/administrator and revoked-credential checks against the public HTTPS endpoint. Verify app routes still return 503 and neither logs nor normal CLI output contain credentials. Record the observed outcomes. Publishing, directory/app authorization, creator removal and integrated infrastructure verification remain #5, #7, #14 and #15.
