...
Catalog version 1 describes only delivered commands, including permissions, inputs, output contracts and effects. Setup status and split login are implemented in #32. Use `small-cloud guide runtime` to learn the runtime contract and `small-cloud app check FOLDER --json` to validate source offline before upload (#33); success does not establish remote build feasibility or readiness. Deployment waits for readiness by default (#34), with explicit `--no-wait` acceptance and bounded, resumable observation; see `small-cloud guide publishing`. Later #29 slices deliver project linking, completion waits for other mutations, live logs and additional workspace commands.
