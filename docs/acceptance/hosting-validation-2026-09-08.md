...
## Estimated spending alerts — 2026-09-08

The operator approved using accessible API metrics for spending alerts, superseding the earlier requirement for provider-metered totals or an invoice before alerting. The control-host collector is active every five minutes and estimates resource costs using current API prices, hourly rounding/monthly caps, reported traffic, retained builder deletion records, quoted VAT and ECB EUR/USD. Hourly monitoring checks estimated month-to-date and month-end forecast thresholds at USD 50/80/100, with explicit coverage and staleness warnings. Runtime monitoring sends health alerts without duplicate project-spending checks.

The first live estimate was USD 0.74 month-to-date and USD 107.57 through September, including the retained temporary-validation upper-bound opening estimate. The forecast excludes future new builds and traffic. The USD 100 forecast alert was accepted by SMTP; new-message recipient receipt is not independently confirmed. [Spending evidence](../evidence/spending-2026-09-08.json) records the report. All 75 infrastructure tests and Pyright passed. Invoice reconciliation remains optional later; these figures are estimates, not actual charges. Independent host-loss observation and unsupported-resource costing remain separate gaps. The operator reports that quota was increased; five-builder capacity still needs live demonstration.
...
| Runtime resource limits | Live pass with competing-limit caveat | Host CPU measurement ~5.27 CPU seconds over ~10.6 wall seconds; 126 guest threads then refusal at 128 total tasks; candidate memory/disk/fork pressure killed the candidate while five serving apps stayed healthy; 1 GiB tmpfs is constrained first by 512 MiB memory |
| Five active apps and update headroom | Live pass for operator harness | Five busy CPU fixtures remained ready while a candidate started in 2.849 seconds; sixth independent app refused; no product promotion/routing workflow claimed |
...
| Thirty deployed apps | Partial | 30 databases are allocated; 30 stored release/image records and product lifecycle are not demonstrated |
...
The existing probe was also rerun on 2026-09-08 with `PROBE_POSTGRES_IMAGE=postgres:16`, matching the control bootstrap's engine major. Both own-database initialization/write/read checks and all six cross-app/maintenance denials passed. The resolved image was `postgres@sha256:f1c3376c26f2609ab9f29f71f824103fe2fcd8ee0346485cb6122a4f93df6f94`; its disposable container was removed by the probe's exit trap. This checks the existing SQL privilege fixture locally, not the new provisioner, deployed TLS, persistence or runtime network policy.
