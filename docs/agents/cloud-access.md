...
The operator chose to wait for CX capacity and authorized automatic procurement of the permanent CX33/CX43 pair once available in Germany. A one-minute workstation cron watcher is installed from a frozen snapshot at `/home/john/.local/lib/small-cloud/cx-watch/`; its first check reported unavailable capacity and no hosts. It enforces EUR 24.48/month net compute plus EUR 1/month net IPv4 ceilings and corresponding hourly limits, retains partial/successful procurement, and emails completion or blockers. See [watcher operations](../../infra/CX-WATCH.md) for status, stopping and recovery. Do not run concurrent manual foundation provisioning. The watcher requires this workstation online; procurement does not configure applications or open creator admission. No invoice is available yet; billing evidence remains pending without blocking local implementation.

Resend sending domain `small-cloud.monkeysees.one` is verified in `eu-west-1`; its DKIM and return-path SPF/MX records are installed in Cloudflare. Receiving, open tracking and click tracking are disabled; delivery TLS is enforced. Private SMTP settings are saved outside Git in `/home/john/.config/small-cloud/secrets/monitor-smtp.json` (mode `0600`): `smtp.resend.com:2587`, STARTTLS, username `resend`, sending-only key, sender `alerts@small-cloud.monkeysees.one`. The existing monitor sender submitted one workstation test to the operator recipient; Resend reports `delivered` for message `83b1c9ad-dc98-4694-b09a-bc68144a1e38`. The operator confirmed receipt on 2026-09-08. This resolves sender authority; host installation and end-to-end operational/spending alerts remain pending because no foundation hosts are running.
