...
The operator subsequently approved permanent CPX32/CPX42 deployment in Germany and cancelled the CX watcher. The higher quoted cost—EUR 105.98/month net including two IPv4 addresses, before builders and other usage—is accepted for now; optimization is deferred. Earlier statements below limiting CPX to temporary validation or prohibiting permanent substitution are historical and superseded by this explicit decision.

The operator selected Hetzner and authorized infrastructure setup in [#17](https://github.com/monkeysees/small-cloud/issues/17). The [permanent-foundation acceptance report](acceptance/hosting-foundation-2026-09-08.md) now records the demonstrated isolation, residency and capacity boundary, current full cost model and timebox assessment. This supersedes the selection hold and incomplete infrastructure findings in the historical assessment below. Creator admission remains closed pending product integration. Use the operator's `small-cloud` project in Germany and `small-cloud.monkeysees.one`, with DNS managed in the existing Cloudflare zone.

For each build, create a fresh x86 VM, preferring CX23 across the EU locations (`nbg1`, `fsn1`, `hel1`) before falling back to CPX22 across those locations. The operator expanded builder placement from Germany to any EU location on 2026-09-08; the permanent control/runtime pair remains in Germany. Both builder sizes provide 2 vCPUs and 4 GiB RAM. Do not fall back outside the EU or to another architecture. Start without a prewarmed pool or shared build cache. A prepared, secret-free builder image may reduce bootstrap work, but does not reserve provider capacity.
...
Frankfurt Linux/x86 Fargate rates read from the [AWS regional price list](https://pricing.us-east-1.amazonaws.com/offers/v1.0/aws/AmazonECS/current/eu-central-1/index.json) on the research date are $0.04656/vCPU-hour and $0.00511/GiB-hour. Fargate requires at least 1 GiB with 0.5 CPU, so its six reserved allocations exceed the proposed 512 MiB app limit and cost `6 × 730 × (0.5 × 0.04656 + 1 × 0.00511) = $124.35`. The [pricing page](https://aws.amazon.com/fargate/pricing/) confirms the CPU/memory combinations and included 20 GB ephemeral storage. Further rows below are explicit planning allowances pending a regional calculator export, not verified Frankfurt quotes.
...
The next decision requires a bounded run in the intended EU deployment environment: independently probe build and runtime isolation, positive public/database controls, forbidden private/metadata/management paths, cross-database authentication, five active apps plus update headroom, failure-preserving deployment, and actual resource placement. Record unavailable and inconclusive checks as such. Until those pass and an available-host quote fits the envelope, this research supports a **no-go for infrastructure selection**, not a relaxation of isolation, residency or the spending target.

## Implementation quote refresh — 2026-09-08

The account API currently quotes the approved CX33/CX43 permanent pair at EUR 24.48/month net, but neither is advertised as available in Germany. CPX32/CPX42 retain the proposed CPU/RAM capacity and cost EUR 104.98/month net for permanent compute alone; they are not authorized automatic replacements. The operator expanded builders to any EU location; current Hetzner options are Germany and Finland. CPX22 is advertised in both countries while CX23 is unavailable. Query `server_types.locations.available`; the older datacenter fields are deprecated and can disagree with current availability.

With the approved permanent pair, two permanent IPv4 addresses add EUR 1/month. At one billed hour per disposable CPX22 plus one IPv4, 100 builds add EUR 3.20; 1,000 builds add EUR 32.00. Including illustrative EUR 2 notification delivery and EUR 10 contingency gives EUR 40.68 or EUR 69.48/month respectively. These remain estimates: no VMs, invoices, traffic or notifications were exercised. Host disks remain the proposed storage allocation, with no volume/snapshot/backup fees assumed; adding a volume costs EUR 0.0572/GiB-month net at the current quote. The update allocation stays inside the runtime VM and does not require another permanent server. Traffic assumes 100 GB within included EU traffic, no paid support, and the existing domain.

The account reports 0% VAT in its quote; operator tax treatment and a current EUR/USD conversion must be established for comparison with the USD 100 target. At EUR 69.48, the target is crossed above USD 1.4393/EUR before tax, or USD 1.1994/EUR at illustrative 20% tax. The notification amount is an allowance until a sender is chosen. Short builds, failed starts, long cleanup or extra billable hours can exceed the modeled build count/cost; this is no cap. Operational labor is excluded from monetary totals and the earlier 9–14 engineering-day estimate remains unvalidated. The breadth of outstanding infrastructure acceptance leaves the two-week timebox unproven.

Live temporary validation subsequently confirmed successful CX23 provisioning in Germany, while a second concurrent builder received Hetzner HTTP 403 `resource_limit_exceeded`. Advertised availability does not establish account quota. The five-build capacity gate therefore needs a quota change and a fresh overlap test. The CPX32/CPX42 pair was authorized only for a two-hour validation session; its higher monthly price is not an accepted permanent substitution. See the dated deployment evidence for temporary charges and cleanup rather than treating this list-price model as an invoice.

Temporary validation ended on 2026-09-08 at 02:40 UTC with an empty Hetzner project inventory and the temporary DNS record removed. Recorded lifetimes plus account list prices estimate EUR 0.4454–0.4678 net for compute and primary IPv4 across both foundation hosts and 11 builders; the range covers one unrecorded early builder type. Extra traffic/tax/service charges and the invoice remain unverified. This one-session figure is separate from the monthly operating model above; see [final cleanup and cost evidence](evidence/hosting-2026-09-08.json).
