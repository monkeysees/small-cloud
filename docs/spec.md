...
People can use coding agents to create purpose-built software, but publishing it for recurring work with colleagues still requires infrastructure help. Creators need a way to publish from their existing coding workflow, control who can use a tool, and update it without routinely losing its data. Colleagues need a discoverable, authenticated place to use that software.
...
Provide one company workspace in the operator's cloud account. Explicitly admitted creators use an agent-callable CLI to upload a local source folder, build a Dockerfile remotely, and publish a tool at a stable URL. A tool runs as one Linux HTTP container with its own logical database and optional creator-configured runtime secrets.

Tools initially admit only their creator. The creator can share with the whole workspace, whose explicitly admitted members can then read and modify all tool data. Google sign-in and platform-enforced authorization protect every tool route. A minimal directory helps members find accessible tools.
...
2. As a creator, I want to upload a local source folder using an agent-callable CLI, so that publishing fits my existing tools.
3. As a creator, I want an isolated remote Dockerfile build, so that publishing does not require local Docker.
4. As a creator, I want to use any language or framework that meets the Linux HTTP container contract, so that templates do not constrain my tool.
...
6. As a creator, I want a published URL and deployment status, so that I can determine whether my tool is available.
...
11. As a creator, I want new tools to be creator-only, so that I can use and inspect them before sharing.
12. As a creator, I want to choose creator-only or workspace-wide sharing, so that I control the audience for my tools.
13. As a workspace member, I want sign-in and tool access checked before my request reaches tool code, so that private tools remain protected.
14. As a tool author, I want the platform to supply a stable user ID, name, and email, so that my tool can recognize the signed-in person.
15. As a workspace member, I want a directory of accessible tools with a name, description, creator, and link, so that I can find relevant software.
16. As a workspace member, I want to read and modify shared tool data, so that colleagues can collaborate without additional per-record permissions.
17. As a creator, I want management permissions limited to my own tools, so that other creators cannot change my deployments or configuration.
18. As a workspace administrator, I want to disable any tool, so that I can stop its availability when necessary.
19. As a creator, I want updates to preserve the URL, sharing scope, and database, so that colleagues can keep returning to the same tool.
...
22. As a tool user, I want an explicit explanation that data has no backup or recovery guarantee, so that I use the pilot only for disposable data.
23. As a tool author, I want a separate logical database and scoped connection credentials, so that my tool cannot access another tool's database.
24. As a tool author, I want to own schema initialization and migrations, so that my tool can evolve its database within the prescribed engine.
...
26. As a creator, I want to reset my database after confirming the tool name, so that I can start over while retaining code, URL, secrets, and sharing scope.
27. As a creator, I want deletion to disable access immediately and schedule removal of artifacts, database, and secrets after confirmation, so that I can retire a tool.
28. As a workspace administrator, I want removing a creator to revoke their platform credentials and disable their tools immediately, so that tools retain an accountable creator.
29. As a tool author, I want public internet access, so that my tool can call external services.
30. As a workspace administrator, I want tools blocked from private networks, metadata endpoints, and platform management services except for their permitted database connection, so that faulty code cannot cross those boundaries.
...
34. As a creator, I want secret changes to restart the tool, so that it uses the saved configuration.
35. As a creator, I want a clear notice when sharing a tool with secrets, so that I understand that its users can trigger the credential-backed actions it exposes.
36. As a creator, I want deployment status, build logs, and runtime logs through the CLI, so that I can diagnose my own tool.
37. As a workspace administrator, I want access to tool diagnostics, so that I can operate the pilot.
...
39. As a tool user, I want idle tools to start on demand with a visible loading state, so that I understand a brief startup delay.
40. As a tool user, I want an immediate capacity message and retry button when all five active slots are occupied, so that I know why my tool cannot start.
41. As a tool user, I want actively used tools to remain running when another tool requests a slot, so that my work is not interrupted to make room.
...
45. As a tool user, I want running tools to remain available after build allowance exhaustion, so that build limits do not interrupt existing work.
46. As a workspace administrator, I want platform content stored and builds and tools executed in EU regions, so that hosting meets the agreed residency boundary.
...
- **Domain and operating boundary:** Support small software classified as tools with disposable data. Serve one company workspace in the operator's cloud account, with company approval assumed. The operator is the sole workspace administrator. Membership and creator privileges are separate grants.
...
- **Runtime contract:** Run one unprivileged Linux HTTP container per tool on a platform-provided port. Supply a connection URL for one prescribed database engine. Container filesystems are temporary; only the database provides persistence. Templates are conveniences and must explain these constraints.
- **Identity and access boundary:** Enforce Google identity, explicit membership, and sharing scope before requests reach tools. Supply stable user ID, name, and email. No publicly accessible container route may bypass enforcement. Trusted identity transport and spoofing prevention are defined in [CLI and platform contracts](contracts.md).
- **Authorization:** Creators manage only their own tools. Ordinary tool users cannot perform platform management operations. The administrator can admit members, grant creator privileges, disable any tool, and inspect diagnostics. Administrative access to creator-only tool content is not specified by the source documents and must not be inferred from diagnostic authority.
- **Sharing and discovery:** Default to creator-only access, with an explicit choice of workspace-wide access. All workspace members can read and modify all data within a workspace-wide tool. The directory lists accessible tools with name, description, creator, and link.
- **Execution and network isolation:** Treat generated code as potentially faulty despite creator admission. Isolate build execution and tool runtimes. Allow public internet traffic while denying private networks, cloud metadata endpoints, and platform management services, with the specific database exception. Metadata must be unreachable; a low-permission metadata identity is insufficient.
- **Database isolation:** Provision a separate logical database and restricted database user per tool. A shared database server is permitted, but one tool's credentials must not authenticate to or access another tool's database. Separate database network endpoints are unnecessary.
- **Secrets:** Encrypt stored values; deliver them only as runtime environment variables. Provide set, replace, delete, and name-list operations with hidden prompts or stdin for values and no saved-value readback. Builds must not receive runtime secrets. Secret changes restart the tool. Tool code is trusted with its own secrets and can disclose them; authorized users can trigger any exposed credential-backed action. Sharing a tool with secrets must explain this authority.
- **Updates and schemas:** Preserve URL, sharing scope, and database on redeployment. Failed builds or startup leave the current container version available. Tools own initialization and migrations; templates demonstrate safe, repeatable startup initialization. Previous-container availability neither reverses schema changes nor guarantees compatibility with a changed database. Recovery may require creator-led schema repair or explicit reset.
- **Lifecycle:** Stop tools after 30 minutes without an HTTP request and start them on demand with a visible loading state. Stopping loses in-memory state. At five active tools, another start request immediately receives a capacity message and retry button; do not evict actively used tools to admit it.
- **Capacity and build accounting:** Support five creators, 30 deployed tools, and five simultaneously active tools. Permit one concurrent build per creator, enforce a ten-minute build timeout, and provide 1,000 shared build minutes per workspace per month. Display usage and refuse further builds when exhausted while keeping running tools available. Enforce runtime-resource limits and refuse new deployments at exhausted capacity. Accounting and UTC month boundaries are defined in [CLI and platform contracts](contracts.md); per-tool resources remain behind the hosting gate.
- **Diagnostics:** Make deployment status, build logs, and runtime logs available through the CLI to the creator and administrator. Logs have seven-day retention, bounded volume, truncation at limits, and known-secret-value redaction. Volume, truncation and redaction-retention limits are defined in [CLI and platform contracts](contracts.md). Redaction does not guarantee confidentiality from tool code.
- **Reset and deletion:** Require explicit confirmation naming the tool for both operations and communicate that recovery is unavailable. Reset erases the database and retains URL, code, secrets, and sharing. Deletion immediately disables access and schedules removal of container artifacts, database, and secrets. Cleanup deadlines, retries and failure visibility are defined in [CLI and platform contracts](contracts.md).
...
- **Isolation:** Run a bounded deployment-environment probe before hosting selection, then retain repeatable checks for database cross-access denial, metadata denial, private-network and management-service denial, permitted database access, and permitted public internet access. Validate build isolation separately from runtime isolation because successful tool requests do not prove build isolation.
- **Secrets:** Verify name-only listing, unavailable saved-value readback, exclusion from builds and source uploads, runtime availability only to the owning tool, replacement and deletion taking effect after restart, sharing notice, and known-value log redaction. Do not assert that malicious tool code cannot disclose entrusted values.
...
- Durable apps, backup or recovery guarantees, automatic database rollback, and a separate migration runner.
- Platform-managed company integrations and per-user third-party authorization.
- Individual tool invitations, group sharing, per-record ownership or roles, and ownership transfer.
...
The product requirements are substantially defined, but this is not an assertion that infrastructure implementation is unblocked. Provider and orchestration selection, database engine and per-tool resources remain open. [CLI and platform contracts](contracts.md) resolve CLI, identity, build accounting, log-volume and cleanup semantics for implementation #3. The two-week timebox is a constraint rather than proof that all requirements fit; the hosting evaluation must establish feasibility without silently weakening accepted isolation or residency requirements.
