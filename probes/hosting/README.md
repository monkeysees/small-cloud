...
For [issue #2](https://github.com/monkeysees/small-cloud/issues/2). These checks exercise the infrastructure boundaries already confirmed in `docs/spec.md`. A local pass is not deployment evidence. Keep the hosting selection gate closed until every deployment check passes.
...
| Another tool's listener/files/secrets | No access in both | Controlled canaries; verify no host mounts, privileged execution or shared build daemon |
