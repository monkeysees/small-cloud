...
The product [idle lifecycle worker](../../identity/LIFECYCLE.md) uses `stop APP` to remove a confirmed idle active allocation without retiring its image or database, and `resume APP DEPLOYMENT --env-file FILE` to launch its retained release through the same capacity and sandbox checks. Both commands hold the runtime host lock. Stop refuses an app with a replacement candidate and is repeatable after a lost response.
