...
```bash
python3 /opt/small-cloud/runtime/sandbox.py --config /etc/small-cloud/runtime.json retain-image app-a release-a sha256:IMAGE_ID
python3 /opt/small-cloud/runtime/sandbox.py --config /etc/small-cloud/runtime.json start app-a sha256:IMAGE_ID
```

Use the [database handoff](../control/README.md) to supply only this app's credentials through a private `/run` environment file, removed after creation. New candidates use `start ... --candidate`; keep the current container and release reference while checking readiness. Product code owns routing promotion, request identity, authorizations and lifecycle state. The harness offers no implicit promotion and never treats a sixth independent app as update headroom.

Both release stores allow thirty apps and two retained releases per app (current plus candidate/previous). On a further update, retire the superseded previous release explicitly. After traffic and allocation removal, retire both references:

```bash
python3 /opt/small-cloud/infra/releases.py retire app-a release-a
python3 /opt/small-cloud/runtime/sandbox.py --config /etc/small-cloud/runtime.json forget-release app-a release-a
```

The hourly registry janitor deletes pending/retired manifests after 23 hours, then stops the registry during garbage collection under the publication lock and restarts it even after an error. Retained releases never age out. Removal is retried idempotently; records disappear only after successful GC. This short registry maintenance window does not stop already-running apps. Runtime cleanup protects retained references and all containers, removes only operator-tracked images with `docker image rm --no-prune` without force, and starts a new one-hour grace when the final reference is forgotten. The `--no-prune` flag is essential: Docker otherwise may delete an untagged parent retained by another release. Its hourly sweep leaves substantial retry time inside the 24-hour cleanup contract. Pending/retired registry records and tracked runtime images each cap at 256; do not bypass a full inventory—repair cleanup. Timers and last oneshot results are monitored. A missed cleanup deadline remains an operational failure, not permission to extend retention.
