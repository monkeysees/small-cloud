...
| --- | --- | --- | --- |
| Linux | x86-64 | `small-cloud-linux-x86_64` | Ubuntu 22.04 / glibc 2.35 |
...
For v0.7.1 onward, the public workflow then builds an undistributed fixture and tests real HTTPS executable replacement plus JSON/human no-update through the resulting production executable. The selected version must be the current public `latest`, and the workflow ref must use that version. The fixture is never uploaded. This same-version fixture-to-production check proves artifact replacement; the controlled native suite separately proves advancement to a newer version. The first installation step still runs before checkout or Python setup.
